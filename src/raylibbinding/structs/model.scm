...
Model MakeModel( Matrix transform, int meshCount, Mesh *meshes, 
                 int materialCount, Material *materials, int *meshMaterial, 
                 int boneCount, BoneInfo *bones, Transform *bindPose )
{
    return (Model){ transform, meshCount, meshes, 
                    materialCount, materials, meshMaterial, 
...
___SCMOBJ SCMOBJ_to_MODEL( ___PSD ___SCMOBJ src, Model *dst, int arg_num )
{
  ...
  if ( !___PAIRP( src ) )
  ...
  else
    {
      ...
      ___SCMOBJ meshes          = ___CADDR( src );
      ___SCMOBJ material_count  = ___CADDDR( src );
